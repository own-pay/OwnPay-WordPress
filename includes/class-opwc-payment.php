...
class OPWC_Payment extends WC_Payment_Gateway
{
	...
	public function init_form_fields()
	{
		...
		$this->form_fields = array(
			'enabled' => array(
				'title' => __('Enable/Disable', 'ownpay-wordpress'),
				'type' => 'checkbox',
				'label' => __('Enable OwnPay Payment', 'ownpay-wordpress'),
				'default' => 'yes'
			),
			'title' => array(
				'title' => __('Title', 'ownpay-wordpress'),
				'type' => 'text',
				'default' => __('OwnPay Payment', 'ownpay-wordpress'),
				'description' => __('This controls the title which the user sees during checkout.', 'ownpay-wordpress'),
				'desc_tip'    => true,
			),
			'custom_logo' => array(
				'title'       => __('Custom Gateway Logo', 'ownpay-wordpress'),
				'type'        => 'image_upload',
				'default'     => '',
				'placeholder' => 'https://example.com/logo.png',
				'description' => __('Upload or choose a custom image to replace the default OwnPay logo on the checkout page. Leave blank to use the default logo.', 'ownpay-wordpress'),
				'desc_tip'    => false,
			),
			'description' => array(
				'title' => __('Description', 'ownpay-wordpress'),
				'type' => 'textarea',
				'default' => __('Pay securely via Cards, Bank Transfer, or Mobile Banking.', 'ownpay-wordpress'),
				'description' => __('This controls the description which the user sees during checkout.', 'ownpay-wordpress'),
				'desc_tip'    => true,
			),
			'api_url' => array(
			...
			'api_key' => array(
				'title' => __('API Key', 'ownpay-wordpress'),
				'type' => 'password',
				'default' => '',
				'description' => __('The Bearer API Key generated in your OwnPay Admin Panel.', 'ownpay-wordpress'),
				'desc_tip'    => true,
			),
			'webhook_secret' => array(
				'title' => __('Webhook Secret', 'ownpay-wordpress'),
				'type' => 'password',
				'default' => '',
				'description' => sprintf(
					__('The shared secret used to verify incoming webhook signatures from OwnPay. You MUST configure this outbound Webhook URL in your OwnPay Merchant Dashboard: %s', 'ownpay-wordpress'),
					'<br/><code>' . esc_url($webhook_url) . '</code>'
				),
				'desc_tip'    => false,
			),
			'add_extra_fee' => array(
				'title' => __('Add Extra Fee', 'ownpay-wordpress'),
				'type' => 'checkbox',
				'label' => __('Enable Extra Fee', 'ownpay-wordpress'),
				'default' => 'no',
				'description' => __('Check this if you want to add an extra charge for paying via OwnPay.', 'ownpay-wordpress'),
				'desc_tip'    => true,
			),
			'fee_percentage' => array(
				'title' => __('Fee Percentage', 'ownpay-wordpress'),
				'type' => 'number',
				'default' => 1.5,
				'custom_attributes' => array(
					'step' => '0.01',
				...
				'description' => __('Percentage fee to charge. E.g. 1.5 for 1.5%.', 'ownpay-wordpress'),
				'desc_tip'    => true,
			),
			'complete_order_after_payment' => array(
				'title' => __('Change Order Status', 'ownpay-wordpress'),
				'type' => 'checkbox',
				'label' => __('Complete order after payment success!', 'ownpay-wordpress'),
				'default' => 'no',
				'description' => __("If enabled, the order status will transition to 'Completed.' Otherwise, it will remain in 'Processing' status.", 'ownpay-wordpress'),
				'desc_tip'    => true,
			),
		);
	}
	...
	public function process_payment($order_id)
	{
		...
		if (!$order) {
			return array(
				'result' => 'failure',
				'messages' => __('Order not found.', 'ownpay-wordpress')
			);
		}
	...
	public function handle_webhook()
	{
		...
		if (empty($raw_body)) {
			status_header(400);
			echo esc_html__('Empty request body.', 'ownpay-wordpress');
			exit;
		}
		...
		if (empty($signature)) {
			status_header(401);
			echo esc_html__('Webhook signature header missing.', 'ownpay-wordpress');
			exit;
		}

		if (empty($this->webhook_secret)) {
			status_header(500);
			echo esc_html__('Webhook secret is not configured in settings.', 'ownpay-wordpress');
			exit;
		}
		...
		if (!hash_equals($expected_signature, $signature)) {
			status_header(403);
			echo esc_html__('Signature verification failed.', 'ownpay-wordpress');
			exit;
		}
		...
		if (!is_array($payload)) {
			status_header(400);
			echo esc_html__('Invalid JSON payload.', 'ownpay-wordpress');
			exit;
		}
		...
		if (!$order) {
			status_header(404);
			echo esc_html__('Order not found matching reference.', 'ownpay-wordpress');
			exit;
		}
		...
		if ($event_type === 'payment.transaction.completed' || $status_lower === 'completed' || $status_lower === 'paid') {
			if (!$order->is_paid()) {
				...
				$order->add_order_note(sprintf(
					__('OwnPay Webhook: Payment completed. Transaction ID: %s. Gateway Transaction: %s.', 'ownpay-wordpress'),
		...
		status_header(200);
		echo esc_html__('Webhook received but event type is ignored.', 'ownpay-wordpress');
		exit;
	}
	...
	public function sync_payment_status($order_id)
	{
		...
		if ($status === 'completed' || $status === 'paid' || $status === 'success') {
			...
			$order->add_order_note(sprintf(
				__('OwnPay Redirect: Payment verified. Transaction ID: %s. Gateway Transaction: %s.', 'ownpay-wordpress'),
				esc_html($trx_id),
				esc_html($gateway_trx_id)
			));
		}
